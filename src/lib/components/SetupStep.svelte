<script lang="ts">
	...
	// You have to be standing at the spot to log an observation there — this is
	// re-checked server-side too (see /api/sessions/complete), since a client
	// gate alone is trivial to bypass.
	const PROXIMITY_THRESHOLD_KM = 0.1;

	type Phase = 'locating' | 'far' | 'gpsError' | 'photo' | 'analyzing' | 'confirm' | 'weatherFallback' | 'details';
	...
	$: ctaCopy = $_('observe.cta', { values: { locality: locality ?? 'Berlin' } });
</script>

{#if phase === 'locating' || phase === 'far' || phase === 'gpsError'}
