...
<div class="absolute inset-0 flex flex-col">
	<div class="flex items-center justify-between border-b border-base-300 px-4 py-3">
		<button type="button" class="btn btn-ghost btn-sm" onclick={cancel}>{$_('space.new.boundary.cancel')}</button>
		<span class="text-sm font-medium text-base-content">{$_('space.new.boundary.label')}</span>
		<button type="button" class="btn btn-primary btn-sm" onclick={done}>{$_('space.new.boundary.done')}</button>
	</div>
